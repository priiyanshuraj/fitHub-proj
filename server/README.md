...
The backend framework chosen for fitHUb is Django, a high-level framework based on Python. In Django, we create database schemas and API endpoints to interact with the frontend, which is the client side of the application.

For data storage, fitHub uses a PostgreSQL database structure hosted and managed through Supabase. This choice allows for efficient data management and scalability.
